  ...
  <div class="articulo-container">
    ...
    <div v-else class="card-grid">
      <div class="articulo-card" v-for="articulo in articulos" :key="articulo.id">
        <div class="contenido">
          <p class="titulo">{{ articulo.descripcion }}</p>
          <p>Precio: ${{ articulo.precio }}</p>
          <p>Stock: {{ articulo.stock }}</p>
          <p>Marca: {{ articulo.marca.nombre }}</p>
          <p>Proveedor: {{ articulo.proveedor.nombre }}</p>
          <p>Categorías: {{ articulo.categorias.map(c => c.nombre).join(', ') }}</p>
        </div>
...
const articulos = computed(() => articuloStore.articulos)
const loading = computed(() => articuloStore.loading)
const error = computed(() => articuloStore.error)
